...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App']).agg({    
            'Reviews':'sum',  
            'Rating':'mean'
            }).reset_index().sort_values(by='Reviews', ascending=False).head(10).set_index('Category').reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    df2.pivot(columns='Content Rating', values = 'Rating').plot.box(figsize=(7,7),flierprops=green_diamond)
